...
class Pix2PixBraTS17CrossModalitiesModel(BaseModel):
    def name(self):
        return 'Pix2PixBraTS17CrossModalitiesModel'
    ...
    def compute_ssim_B(self):
        real_B = self.real_B.data.cpu().numpy()[:, 1, :, :]
        fake_B = self.fake_B.data.cpu().numpy()[:, 1, :, :]
        value = ssim(real_B, fake_B, data_range=fake_B .max() - fake_B .min())
        print(value)
    def forward(self):
