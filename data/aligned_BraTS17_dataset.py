...
class AlignedBraTS17Dataset(BaseDataset):
    @staticmethod
    ...
    def __getitem__(self, index):
        ...
        w2 = int(w / 3)
        w3 = w2 * 2
        A = ABC.crop((0, 0, w2, h)).resize((self.opt.loadSize, self.opt.loadSize), Image.BICUBIC)
        B = ABC.crop((w2, 0, w3, h)).resize((self.opt.loadSize, self.opt.loadSize), Image.BICUBIC)
        C = ABC.crop((w3, 0, w, h)).resize((self.opt.loadSize, self.opt.loadSize), Image.BICUBIC)
        ...
        return {'A': A, 'B': B, 'C': C,
                'A_paths': ABC_path, 'B_paths': ABC_path, 'C_paths': ABC_path}

    def __len__(self):
        return len(self.ABC_paths)

    def name(self):
        return 'AlignedBraTS17Dataset'
